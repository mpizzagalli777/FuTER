...
open(my $DBG, '>', 'debug.tsv') or die "Cannot write debug.tsv: $!";
print $DBG join("\t", qw(read_id min_alt_exon_delta max_alt_exon_delta minDeltaA minDeltaB)), "\n";
...
sub parse_chims {
    ...
    while (<$fh>) {
        ...
        my $has_TE = ($chrA_n_coordA =~ /^chrTE:/) || ($chrB_n_coordB =~ /^chrTE:/);
        my $teA = ($chrA_n_coordA =~ /^chrTE:/) ? 1 : 0;
        my $teB = ($chrB_n_coordB =~ /^chrTE:/) ? 1 : 0;
...
sub filter_chims {
    ...
    foreach my $fusion_candidate (@$fusion_candidates_aref) {
        ...
        if ($ffpm >= $min_FFPM
            ...
            # deltas within range on either side and TE so MAX delta is increased. Requires higher read support
                ( $fusion_candidate->{has_TE} && $ffpm >= 1 && 
                	(( $fusion_candidate->{teA} && 
                	$fusion_candidate->{min_deltaA} <= 10 * $ALT_MAX_EXON_DELTA && 
                	$fusion_candidate->{min_deltaB} <= $MAX_EXON_DELTA )
                	||
                	( $fusion_candidate->{teB} && 
                	$fusion_candidate->{min_deltaB} <= 10 * $ALT_MAX_EXON_DELTA && 
                	$fusion_candidate->{min_deltaA} <= $MAX_EXON_DELTA ))
              )
            ...
            my $rid = (ref $fusion_candidate->{read_names} eq 'ARRAY') ? join(",", @{ $fusion_candidate->{read_names} }) : $fusion_candidate->{read_names};

            # log exactly what you asked for: min_alt, max_alt_exon_delta, and the read
            print $DBG join("\t",
                    		$rid,
                    		(defined $min_alt_exon_delta  ? $min_alt_exon_delta  : 'NA'),
                    		(defined $max_alt_exon_delta  ? $max_alt_exon_delta  : 'NA'),
                    		$fusion_candidate->{min_deltaA},
                    		$fusion_candidate->{min_deltaB}
           			), "\n";
        }
    }
