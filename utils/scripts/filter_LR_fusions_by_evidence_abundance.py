...
def main():
    ...
    parser.add_argument("--te_only", action="store_true", help='Keep only fusions whose "#FusionName" contains "chrTE"')
    ...
    fusion_col = "#FusionName" if "#FusionName" in data.columns else (
             "FusionName"  if "FusionName"  in data.columns else None)
    if fusion_col is None:
    	raise SystemExit('Input is missing a FusionName column ("#FusionName" or "FusionName").')
    ...
    if args.te_only:
    	pre = len(data_filtered)
    	data_filtered = data_filtered[
        	data_filtered[fusion_col].astype(str).str.contains("chrTE", na=False)
    	]
    	logger.info("TE-only filter: kept %d of %d rows", len(data_filtered), pre)
