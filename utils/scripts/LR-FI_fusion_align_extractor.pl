    ...
    foreach my $scaffold (keys %scaffold_to_gene_coordsets) {
        ...
        foreach my $LR_acc (@LR_accs) {
            ...
            if ($min_LR_coord <= $geneA_max && $max_LR_coord >= $geneB_min) { # spans both genes 
                ...
                if ($left_gene_overlapped_bases < $min_trans_overlap_length || ($left_gene =~ /^chrTE/ && $left_gene_overlapped_bases < $min_trans_overlap_length * 0.66)) {
                    if ($DEBUG) {
                ...
                if ($right_gene_overlapped_bases < $min_trans_overlap_length || ($right_gene =~ /^chrTE/ && $right_gene_overlapped_bases < $min_trans_overlap_length * 0.66)) {
                    if ($DEBUG) {
                ...
                unless (defined $break_left && defined $break_right) {
    				print $filtered_ofh "$scaffold\t$LR_acc\tno valid breakpoint found with gene bounds ($geneA_max,$geneB_min)\n";
    				next;  # skip this read, continue with the rest
				}
