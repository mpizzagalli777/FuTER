...
sub filter_remaining_promiscuous_fusions {
    ...
    foreach my $fusion (@$fusions_aref) {
        ...
        my $geneA_is_TE = ($geneA =~ /^chrTE/);
        my $geneB_is_TE = ($geneB =~ /^chrTE/);
        
        if ($geneA_is_TE && $geneB_is_TE) {
			$row->{FilterReason} = 
           		  "FILTERED DUE TO TE - TE fusion.";
        	$filter_ofh_writer->write_row($row);
        	 # Do not push to @ret_fusions (filtered out)
        	next;
		}
		
        print STDERR "Fusion: $geneA--$geneB, num_A_partners: $num_geneA_partners, num_B_partners: $num_geneB_partners\n" if $DEBUG;
        
        if ((! $EXCLUDE_LOCI_OVERLAP_CHECK) && &is_promiscuous($num_geneA_partners, $num_geneB_partners, $max_promiscuity)) {
        ...
        if (&is_promiscuous($num_geneA_partners, $num_geneB_partners, $max_promiscuity)) {
			...
			my $geneA_prom = ($num_geneA_partners >= $max_promiscuity);
			my $geneB_prom = ($num_geneB_partners >= $max_promiscuity);
	    	
	    	#If only the TE is promiscuous, keep the fusion transcript	
			if (($geneA_is_TE && $geneA_prom && ! $geneB_prom) || ($geneB_is_TE && $geneB_prom && ! $geneA_prom)) {
	    		    
				# Append "promiscuous TE" to annots field
				my $annots_field = $row->{annots};
				
				if (!defined $annots_field || $annots_field eq "" || $annots_field eq "[]") {
				
					$row->{annots} = '["promiscuous TE"]';
					
				} else {
					$annots_field =~ s/\]\s*$//; 
					$row->{annots} = $annots_field . ',"promiscuous TE"]';
				}
				push @ret_fusions, $fusion;
				next;  # skip to next fusion (do not mark as filtered)
			} 
			   
       		# For all other promiscuous cases (no TE involved, or gene partner also promiscuous):
        	$row->{FilterReason} = 
           		"FILTERED DUE TO reached max promiscuity ($max_promiscuity), "
           		. "num_partners($geneA)=$num_geneA_partners and num_partners($geneB)=$num_geneB_partners";
        	$filter_ofh_writer->write_row($row);
        	# Do not push to @ret_fusions (filtered out)
        	next;
       		...
       		# Not promiscuous, keep the fusion
       		push @ret_fusions, $fusion;
    	}
    }
    return(@ret_fusions);
