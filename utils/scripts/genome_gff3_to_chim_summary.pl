...
#
#  --min_per_id <float>        minimum percent identity for genes (default: $min_per_id)
#
#  --min_per_id_TE <float>     minimum percent identity of TEs (default: $min_per_id_TE)
#
#  *  optional:
#
#  --TE_only                   report only TE fusions
#
    ...
    foreach my $target (keys %target_to_aligns) {
        ...
        foreach my $span_id (@span_ids) {
            ...
            eval {
                ...
                if ($span_struct->{chr} =~ /^chrTE/ && $span_struct->{per_id} >= $min_per_id_TE) {
                    push (@spans, $span_struct);
                }

                elsif ($span_struct->{per_id} >= $min_per_id) {
                    push (@spans, $span_struct);
                }
            };
            if ($@) {
            ...
            if ($prev_align) {
                ...
                foreach my $left_possibility (@left_possibilities) {
                    ...
                    foreach my $right_possibility (@right_possibilities) {
                        ...
                        my $left_is_te  = $left_entry->{chr}  =~ /^chrTE/;
                        my $right_is_te = $right_entry->{chr} =~ /^chrTE/;
                        
                        if (!($left_is_te || $right_is_te)) {
                            # gene–gene: must agree strand, and swap if antisense
                            unless ($left_entry->{sense_or_antisense}
                                eq $right_entry->{sense_or_antisense})
                            {
                                next;
                            }
                            ...
                            # TE–gene: let orientation of the gene drive the swap
                            my $gene_entry = $left_is_te ? $right_entry : $left_entry;
                            # if that gene piece aligned antisense, swap so gene is sense
                            if ($gene_entry->{sense_or_antisense} eq 'antisense') {
                                ($left_entry, $right_entry) = ($right_entry, $left_entry);
                            }
                            # now left_entry is the gene portion and in sense orientation
                        }
                        if ($left_is_te && $right_is_te) {
                        	#Skip TE-TE fusions
                        	next;
        ...
        if (@fusion_preds) {
            ...
            while (@fusion_preds) {
                ...
                if ($pred->{delta_sum} == $min_delta_sum) {
                    # If TE_only is set, require at least one partner to be on chrTE_*
                    if ($TE_only) {
                        my $left_chr  = $pred->{left_entry}->{chr}  // '';
                        my $right_chr = $pred->{right_entry}->{chr} // '';
                    
                        my $left_is_te  = ($left_chr  =~ /^chrTE/);
                        my $right_is_te = ($right_chr =~ /^chrTE/);
                    
                        # skip non-TE fusions
                        next unless ($left_is_te || $right_is_te);
                    }
