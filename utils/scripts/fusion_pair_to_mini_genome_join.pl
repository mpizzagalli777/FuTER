...
my %contig_len;
{
    my $fai = $genome_fasta_file . ".fai";
    open(my $FH, "<", $fai)
      or die "Error, cannot open fasta index $fai";
    while(<$FH>) {
        chomp;
        my ($chr, $len) = split(/\t/);
        $contig_len{$chr} = $len;
...
sub get_gene_contig_gtf {
    ...
    my $start = $gene_lend - $genome_flank_size;
    $start = 1 if $start < 1;
    my $end   = $gene_rend + $genome_flank_size;
    ...
    my $gene_contig_gtf = &transform_gtf_coordinates($start,
                                                     $gene_gtf,
                                                     length($seq_region),
                                                     $gene_orient);
...
sub get_genomic_region_sequence {
    ...
    if ( $chr =~ /^chrTE_/ ) {
        # never ask for <=0
        $lend = 1 if $lend < 1;
        # never ask past end of scaffold
        if (exists $contig_len{$chr} && $rend > $contig_len{$chr}) {
            $rend = $contig_len{$chr};
        }
    }
